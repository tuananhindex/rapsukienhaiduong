...
class MenuController extends Controller
{
    ...
    public function add_post(Request $req){
        ...
        $data['menu_footer'] = $req->menu_footer;
        $data['menu_header'] = $req->menu_header;
    ...
    public function edit_get($id){
        ...
        $data_cursor = !empty($index->cursor) ? DB::table($index->cursor)->select('id','name','alias')->get() : false;
        //return dd($data_cursor);
        $MultiLevelSelect = AdminHelper::MultiLevelSelect($cats,0,'',$index->fk_parentid);

    	return view($this->e['view'].'.edit',compact('index','cats','MultiLevelSelect','data_cursor'))->with(['e' => $this->e]);
    }
